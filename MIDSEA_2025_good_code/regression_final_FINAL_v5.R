# load df
df <- read.csv("MIDSEA_2025_good_code/analysis_new/data_from_email.csv")

# descriptive
hist(df$math_score)
df_density <- density(df$math_score)
plot(df_density$x, df_density$y, type = "l", main="Math score distribution")
temp_mean <- mean(df$math_score)
df_deviations <- sd(df$math_score)

y <- dnorm(seq(0,100,by=0.1), df_density, df_deviations)
lines(seq(0,100,by=0.1),y, col="red")
